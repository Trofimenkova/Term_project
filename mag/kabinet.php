...
inner join статусы_заказов on заказы.Id_статус = статусы_заказов.Id_статус where username='".$_SESSION["session_username"]."'";       
        ...
        while($order = mysqli_fetch_array($rezult)) { ?>
						<tr>
							...
							<td class="t">
							<?php 
						 $query2 = "select Id_заказ, заказ_товар.Id_товар, Вид, заказ_товар.Количество as Заказанное_количество, Цена from заказ_товар inner join товары
