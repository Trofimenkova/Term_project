<footer id="footer">
		<div class="container">
			<div class="cols">
				<div class="col">
					<h3>Сервис и поддержка</h3>
					<ul>
						<li><a href="about.php#about">О магазине</a></li>
						<li><a href="about.php#return">Возврат товара</a></li>
						<li><a href="dostavka.php#dostavka">Доставка</a></li>
						<li><a href="dostavka.php#payment">Способы оплаты</a></li>
					</ul>
				</div>
				<div class="col media">
					<h3>Мы в социальных сетях</h3>
					<ul class="social">
						<li><a href="#"><span class="ico ico-fb"></span>vk.com</a></li>
						<li><a href="#"><span class="ico ico-tw"></span>skype</a></li>
						<li><a href="#"><span class="ico ico-gp"></span>telegram</a></li>
					</ul>
				</div>
				<div class="col contact">
					<h3>Контакты</h3>
					<p><span class="ico ico-em"></span><a href="#">anna.trof@tut.by</a></p>
					<p><span class="ico ico-ph"></span>+375(29)673-67-60</p>
					<p><span class="ico ico-ph"></span>+375(29)187-08-42</p>
				</div>
				<div class="col newsletter">
					<script type="text/javascript" src="//vk.com/js/api/openapi.js?142"></script>

					<!-- VK Widget -->
					<div id="vk_groups"></div>
					<script type="text/javascript">
					VK.Widgets.Group("vk_groups", {width: 300, mode: 3}, 14168212);
					</script>
				</div>
			</div>
			<p class="copy">&copy Рыбин Гуд 2017</p>
		</div>
		<!-- / container -->
	</footer>
	<!-- / footer -->